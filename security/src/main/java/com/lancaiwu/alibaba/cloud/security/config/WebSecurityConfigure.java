...
//    	web.ignoring().antMatchers("/connect/**"); //无条件允许访问
        // web.ignoring().antMatchers("/common/**", "/security-manage/user-manage/add"); //无条件允许访问
    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        // url权限认证处理
        http.authorizeRequests().antMatchers("/common/**", "/security-manage/user-manage/add").permitAll()
...
//                })
         ...
         * 默认它提供了三种登录方式
         * 1、formLogin() 普通表单登录
         * 2、oauth2Login() 基于 OAuth2.0 认证/授权协议
         * 3、openidLogin() 基于 OpenID 身份认证规范
         */
        // 开启自动配置的登录功能
        http.formLogin() //开启登录
