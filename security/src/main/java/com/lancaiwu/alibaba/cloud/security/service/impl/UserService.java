package com.lancaiwu.alibaba.cloud.security.service.impl;
...
import com.lancaiwu.alibaba.cloud.security.config.UrlResponse;
import com.lancaiwu.alibaba.cloud.security.mapper.AuthorityUserRepository;
import com.lancaiwu.alibaba.cloud.security.utils.TimeUtil;
...
public class UserService {
     ...
     * 新增用户信息
     */
    public void addUserInfo(AuthorityUser authorityUser) {
        String encode = new BCryptPasswordEncoder().encode(authorityUser.getPassword());
        long timeId = System.currentTimeMillis();
        String nowTime = TimeUtil.FORMAT.get().format(timeId);
        if (ObjectUtils.isEmpty(authorityUser.getValidTime())) {
            String validTimeDefault = TimeUtil.FORMAT.get().format(timeId + 7 * 24 * 60 * 60 * 1000);
            authorityUserRepository.addUserInfo(authorityUser.getNickname(), authorityUser.getUsername(), encode, authorityUser.getEmail(), authorityUser.getPhone(), validTimeDefault, nowTime, authorityUser.getRemark());
        } else {
            authorityUserRepository.addUserInfo(authorityUser.getNickname(), authorityUser.getUsername(), encode, authorityUser.getEmail(), authorityUser.getPhone(), authorityUser.getValidTime(), nowTime, authorityUser.getRemark());
        }
    }
